...
Details on Buster can be found [here](https://github.com/axitkhurana/buster). Basically, you need to install **wget** and **git** with [brew](https://brew.sh/) (if they are not already installed on your system) then install Buster with [pip](https://pip.pypa.io).  Just run the following commands:
...
At the time of writing this Buster had some issues with missing the generation of some needed files and creating some files with incorrect URL reference in page. I created this script as a workaround for those issue.  So create a file (I called it **build-static.sh**) and be sure and change out the references to https://joshgerdes.com and replace them with your remote site URL.
...
find static/* -name robots.txt -type f -exec sed -i '' 's#http://localhost:2368#https://joshgerdes.com#g' {} \;
find static/* -name *.xsl -type f -exec sed -i '' 's#http://localhost:2368#https://joshgerdes.com#g' {} \;
find static/* -name *.xml -type f -exec sed -i '' 's#loc>http://localhost:2368#loc>https://joshgerdes.com#g' {} \;
find static/* -name *.html -type f -exec sed -i '' 's#u=http://localhost:2368#u=https://joshgerdes.com#g' {} \;  
find static/* -name *.html -type f -exec sed -i '' 's#url=http://localhost:2368#url=https://joshgerdes.com#g' {} \;  
find static/* -name *.html -type f -exec sed -i '' 's#href="http://localhost:2368#href="https://joshgerdes.com#g' {} \;  
find static/* -name *.html -type f -exec sed -i '' 's#src="http://localhost:2368#src="https://joshgerdes.com#g' {} \;  
find static/* -name *.html -type f -exec sed -i '' 's#link>http://localhost:2368#link>https://joshgerdes.com#g' {} \; 
